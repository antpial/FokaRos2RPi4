...
class sensor_current_driver:
    def __init__(self,*, adc_channel):
        ...
        if adc_channel < 0 or adc_channel > 3:
            raise ValueError("sensor_current: adc_channel must be between 0 and 3 ")
    ...
    def read_data_Float32(self):
        ...
        a = self._scale_current(voltage)
        print(f"Napięcie: {voltage:.3f} V, Prad: {a:.1f} A")
        return Float32(data=a)
