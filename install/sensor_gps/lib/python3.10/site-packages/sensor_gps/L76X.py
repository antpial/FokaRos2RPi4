...
class L76X(object):
    ...
    def L76X_Gat_GNRMC(self):
                ...
                if '$GNGGA' in sentence:
                    ...
                    if len(parts) > 8 and parts[8]:
                        try:
                            ...
                            self.EstimatedAccuracy = self.HDOP * 5.0
                            # print(f"HDOP: {self.HDOP}, Estimated Accuracy: {self.EstimatedAccuracy:.2f} m")
                        except ValueError:
                            # print(f"Błąd konwersji HDOP: {parts[8]}")
                            pass
                self.Satellites = g.satellites_in_use
                self.SpeedMh = g.speed[0]
        ...
        self.Lat = int(g.latitude[0]) + (g.latitude[1] / 60)
        self.Lon = int(g.longitude[0]) + (g.longitude[1] / 60)
